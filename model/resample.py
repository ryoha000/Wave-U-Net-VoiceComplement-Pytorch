...
class Resample1d(nn.Module):
    def __init__(self, channels, kernel_size, stride, transpose=False, padding="reflect", trainable=False):
        ...
        self.filter = torch.nn.Parameter(torch.from_numpy(np.repeat(np.reshape(
            filter, [1, 1, kernel_size]), channels, axis=0)), requires_grad=trainable)
    ...
    def forward(self, x):
        ...
        if self.transpose:
            ...
            out = F.conv1d(out, self.filter, stride=self.stride,
    ...
    def get_output_size(self, input_size):
        ...
        if self.transpose:
            ...
            # Want to take first and last sample
            assert(input_size % self.stride == 1)
            if self.padding == "valid":
    ...
    def get_input_size(self, output_size):
        ...
        # Transposed
        if self.transpose:
            # We need to have a value at the beginning and end
    ...
    for i in range(kernel_size):
        if i == M//2:
            ...
            filter[i] = (np.sin(2 * np.pi * cutoff * (i - M//2)) / (i - M//2)) * \
                (0.42 - 0.5 * np.cos((2 * np.pi * i) / M) +
                 0.08 * np.cos(4 * np.pi * M))
