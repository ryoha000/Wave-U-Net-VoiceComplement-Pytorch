...
class UpsamplingBlock(nn.Module):
    ...
    def get_output_size(self, input_size):
        ...
        # Upsampling convs
        for conv in self.pre_shortcut_convs:
            curr_size = conv.get_output_size(curr_size)  # type: ignore
...
class DownsamplingBlock(nn.Module):
    ...
    def get_input_size(self, output_size):
        ...
        for conv in reversed(self.post_shortcut_convs):
            curr_size = conv.get_input_size(curr_size)  # type: ignore
...
class Waveunet(nn.Module):
    ...
    def set_output_size(self, target_output_size):
        ...
        self.input_size, self.output_size = self.check_padding(
            target_output_size)
        print("Using valid convolutions with " + str(self.input_size) +
              " inputs and " + str(self.output_size) + " outputs")
    ...
    def check_padding(self, target_output_size):
        ...
        while True:
            out = self.check_padding_for_bottleneck(
                bottleneck, target_output_size)
            if out is not False:
                return out
            bottleneck += 1
    ...
    def check_padding_for_bottleneck(self, bottleneck, target_output_size):
        ...
        try:
            ...
            # block は Module (module.upsampling_blocks は ModuleList)
            for idx, block in enumerate(module.upsampling_blocks):  # type: ignore
                curr_size = block.get_output_size(curr_size)
            output_size = curr_size

            # Bottleneck-Conv
            curr_size = bottleneck
            for block in reversed(module.bottlenecks):  # type: ignore
                curr_size = block.get_input_size(curr_size)
            for idx, block in enumerate(
                    reversed(module.downsampling_blocks)):  # type: ignore
                curr_size = block.get_input_size(curr_size)
    ...
    def forward(self, x, inst=None):
        ...
        return self.forward_module(x, self.waveunets['ALL'])
